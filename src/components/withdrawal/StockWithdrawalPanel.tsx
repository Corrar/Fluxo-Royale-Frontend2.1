...
// ✨ IMPORTANTE: Importamos o nosso Fallback Offline para a Mutação
...
export function StockWithdrawalPanel() {
  ...
  // 1. Buscar Produtos Reais da API (Isto será guardado na cache pelo Service Worker)
  ...
  // 2. ✨ Mutação Atualizada para Suportar o Modo Offline
  const withdrawalMutation = useMutation({
    mutationFn: async (data: { sector: string; items: { product_id: string; quantity: number }[] }) => {
      // Substituímos o api.post por apiWithOfflineFallback
      const resultado = await apiWithOfflineFallback(
        'POST',
        '/manual-withdrawal',
        data,
        'retirada', // Tipo de ação
        'estoque'   // Contexto
      );
      return resultado;
    },
    onSuccess: (resultado) => {
      // Validamos se o resultado foi guardado offline ou se a chamada foi online
      if (resultado && resultado.offline) {
        toast.warning("Sem internet! Retirada guardada e será sincronizada quando a rede voltar.", {
          duration: 5000
        });
      } else {
      ...
      // Atualizamos a cache de imediato
